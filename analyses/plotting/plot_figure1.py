...
def main():
    ...
    panel_a = sg.fromfile(str(_root / "typst/plots/fig1" / "pipeline_schematic.svg"))
    panel_bc = sg.fromfile(str(sankey_gene_path))

    # Scale panel A to match the width of panel BC
    bc_w = float(panel_bc.width.replace("pt", ""))
    a_w = float(panel_a.width)
    a_h = float(panel_a.height)
    scale = bc_w / a_w
    a_h_scaled = a_h * scale

    bc_h = float(panel_bc.height.replace("pt", ""))
    total_h = a_h_scaled + bc_h

    combined = sg.SVGFigure(f"{bc_w}pt", f"{total_h}pt")
    a_root = panel_a.getroot()
    a_root.scale(scale)
    a_root.moveto(0, 0)
    bc_root = panel_bc.getroot()
    bc_root.moveto(0, a_h_scaled)
    combined.append([a_root, bc_root])

    out_path = _root / "typst/plots/fig1/fig1.svg"
