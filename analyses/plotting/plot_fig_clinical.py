"""
Supplementary figure: ION582 benchmarking against ASO Atlas 2.0.

Panel A — IC50 distribution (gymnosis/free uptake) split by target gene, ION582 marked.
Panel B — Mouse bFOB distribution (ICV, 700 µg, 3 h) split by target gene, ION582 marked.
Panel C — Rat mFOB distribution (IT, 3000 µg, 3 h) split by target gene, ION582 marked.

Reads: data/oligostack/processed/dose_response_processed.parquet
       data/oligostack/processed/neurotoxicity_processed.parquet
"""
...
OUT_DIR = _root / "typst" / "plots" / "fig_clinical"
...
def _mark_drug(ax, x_val, name, color):
    ylim = ax.get_ylim()
    arrow_top = ylim[1] * 0.35
    ax.annotate(
        "", xy=(x_val, 0), xytext=(x_val, arrow_top),
...
def draw_ic50_panel(dr, ax):
    ...
    if len(target_vals) > 0:
        ax.hist(np.log10(target_vals), bins=bins, color="#4878A8", edgecolor="black",
                linewidth=0.4, alpha=0.85, zorder=3, label=TARGET)

    ion582_ic50 = ic50s.get(ION582["id"])
    if ion582_ic50 and not np.isnan(ion582_ic50):
        _mark_drug(ax, np.log10(ion582_ic50), ION582["name"], ION582["color"])
...
def draw_neuro_panel(neuro, species, ax):
    ...
    if len(rows) > 0:
        ...
        _mark_drug(ax, fob, ION582["name"], ION582["color"])
    ...
    ax.set_ylabel("Count")
    ax.set_title(f"{species} {fob_label} — {detail}", fontsize=10)
    ax.grid(True, alpha=0.2)
...
def main():
    ...
    svg_path = OUT_DIR / "fig_clinical.svg"
