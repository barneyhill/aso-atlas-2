...
BASE_BIOMARKER_JSON = _root / "data/results/base_biomarker.json"
OUT_DIR = _root / "typst/plots/fig_concordance"
...
def _load_compound_metrics():
    ...
    Returns (metrics, id_sets) where:
      metrics = {"iv_max": Series, "dr_ic50": Series, "mouse_alt": Series,
                 "mouse_fob": Series, "rat_alt": Series, "rat_fob": Series}
    ...
    metrics = {
        ...
        "dr_ic50": compound_ic50s(dose_resp),
        "mouse_alt": compound_mean_biomarker(
            hep[hep["species"] == "mouse"], "ALT",
        ),
        "mouse_fob": compound_mean_biomarker(
            neuro[(neuro["species"] == "Mouse") & (neuro["dosage_ug"] == 700)],
            "FOB_score",
        ),
        "rat_alt": compound_mean_biomarker(
            hep[hep["species"] == "rat"], "ALT",
        ),
        "rat_fob": compound_mean_biomarker(
            neuro[(neuro["species"] == "Rat") & (neuro["dosage_ug"] == 3000)],
            "FOB_score",
        ),
    }
    id_sets = {
...
def draw_cross_assay_corr(ax, metrics):
    """Draw 6×6 Spearman correlation matrix across assay stages and species.
    ...
    series = [metrics["iv_max"], metrics["dr_ic50"],
              ...
              metrics["rat_alt"], metrics["rat_fob"]]
    n = len(labels)
...
def compute_base_biomarker_corr():
    ...
    context_keys = ["mouse_alt", "rat_alt", "mouse_bfob", "rat_mfob"]
    contexts = [
        (hep[hep["species"] == "mouse"], "mean_ALT"),
        (hep[hep["species"] == "rat"], "mean_ALT"),
        (neuro[
    ...
    for j, (df, col) in enumerate(contexts):
    ...
    BASE_BIOMARKER_JSON.write_text(json.dumps(results, indent=2) + "\n")

    return rho_matrix, sig_matrix


def draw_base_biomarker_corr(ax):
    """Draw 4×4 heatmap: base composition (A,T,G,C) vs toxicity biomarkers."""
    rho_matrix, sig_matrix = compute_base_biomarker_corr()

    biomarker_labels = ["Mouse\nALT", "Rat\nALT", "Mouse\nbFOB", "Rat\nmFOB"]
    bases = BASES
...
def main():
    ...
    out_path = OUT_DIR / "fig_concordance.svg"
