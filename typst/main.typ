...
For hepatotoxicity, the dinucleotide random forest (128 features, 5,000 trees) was evaluated following the original methodology: out-of-bag estimation and Levenshtein-stratified 10-fold cross-validation on #comma(R.hagerdorn_hepatotox.n) compounds. The model achieved an OOB accuracy of #str(R.hagerdorn_hepatotox.oob_accuracy) and a stratified CV accuracy of #str(R.hagerdorn_hepatotox.accuracy) with an AUC of #str(R.hagerdorn_hepatotox.auc) (sensitivity #str(R.hagerdorn_hepatotox.sensitivity), specificity #str(R.hagerdorn_hepatotox.specificity)). Additional baseline, counts, and positional models were evaluated with GroupKFold cross-validation grouped by target gene.
...
*Label assignment.* Species-specific upper limits of normal (ULN) were taken from published reference intervals as the mean of male and female 97.5th percentiles: 75 IU/L for C57BL/6J mice (Otto _et al._, 2016), 39 IU/L for Sprague-Dawley rats (He _et al._, 2017), and 103 IU/L for adult cynomolgus macaques (Bakker _et al._, 2023). The pipeline pass threshold was set at 2$times$ULN (150 IU/L mouse, 78 IU/L rat, 206 IU/L monkey). For classifier training, compounds with mean ALT $≥$ 2$times$ULN were labelled "high" toxicity; those with ALT < 2$times$ULN were labelled "low".
...
We replicated the Hagedorn _et al._ (2022) approach for predicting ASO neurotoxicity from sequence features, evaluated on mouse bFOB scores from ICV administration at 700 µg.
