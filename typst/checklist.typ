...
+ #claim(
  ...
  answer: answerYes,
  justification: [The abstract and introduction state three contributions (cost-based evaluation framework, ASO Atlas 2.0 dataset, baseline benchmark) that directly correspond to the experimental results in @sec_atlas to @sec_benchmark. Limitations are discussed in @sec_discussion.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [We discuss limitations in @sec_discussion, including the reliance on chemistry-only features without target context, the single-organisation data source (Ionis Pharmaceuticals patents), and the binary classification approach that discards intermediate toxicity cases. No strong assumptions were made about the datasets or models benchmarked in this study.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerNA,
  justification: [The paper does not include formal theoretical results. The cost-based framework is an empirical model, not a theorem.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [@sec_atlas to @sec_benchmark fully describe the extraction pipeline, data cleaning, model features, cross-validation procedure, and cost framework with per-stage costs. All model configurations are specified.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [
    All code and data necessary to reproduce the main experimental results are openly available. The full codebase, including preprocessing, modeling, and evaluation scripts, is released at #url("https://github.com/ASO-Atlas/aso-atlas-2.0"). The underlying dataset is accessible at #url("https://huggingface.co/datasets/barneyhill/aso-atlas-2").
  ],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [We provide an extensive description of the experimental setup in @sec_atlas to @sec_benchmark, including data splits, model features, label thresholds, and hyperparameter selection strategy. All code used to produce and visualise the results will be publicly released.],
  guidelines: [
    - The answer NA means that the paper does not include experiments.
...
+ #claim(
  ...
  answer: answerYes,
  justification: [All model metrics and cost-based evaluation results are reported as mean ± standard deviation across 5 GroupKFold cross-validation folds. The source of variability (fold-level variation) is stated.],
  guidelines: [
    - The answer NA means that the paper does not include experiments.
...
+ #claim(
  ...
  answer: answerYes,
  justification: [The extraction pipeline LLM API cost (~\$500), OligoGym benchmark CPU runtime (~2--4 hours), and OligoAI GPU training time (~25 GPU-hours on NVIDIA RTX A6000) are reported in the paper.],
  guidelines: [
    - The answer NA means that the paper does not include experiments.
...
+ #claim(
  ...
  answer: answerYes,
  justification: [The research uses publicly available patent data and does not involve human subjects, deception, or dual-use risks.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [The paper discusses the positive societal impact of reducing animal use and preclinical screening costs. No direct negative societal impacts are foreseen for a benchmark dataset.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerNA,
  justification: [The paper poses no such risks. The dataset is derived from publicly available USPTO patent filings and the models are small classifiers for ASO property prediction.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [All existing assets are properly cited and their terms of use respected. Patent data is sourced from the USPTO bulk-data API and is in the public domain.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [ASO Atlas 2.0 is documented with extraction pipeline details (@sec_extraction), preprocessing and quality control procedures (@secS_preprocessing), and a dataset summary (@sec_atlas). The code and data are released in a public repository with instructions for reproduction.],
  guidelines: [
...
+ #claim(
  ...
  answer: answerYes,
  justification: [The ASO Atlas 2.0 extraction pipeline uses GPT-5-mini (Stages 1--2) and GPT-5 (Stage 3) as core components, described in detail in @sec_extraction.],
  guidelines: [
